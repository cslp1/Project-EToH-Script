...
local function _initTowerPortal()
    ...
    -- of them for you instead of making you scroll. Any tower currently in workspace is a
    -- candidate -- towers with no resolvable entry portal included, since Automake Route
    -- works on them regardless and they're still somewhere you can walk to.
    local function loadedTowers()
        local towersFolder = workspace:FindFirstChild("Towers")
        if not towersFolder then return nil, "No workspace.Towers in this place." end

        local out = {}
        for _, child in ipairs(towersFolder:GetChildren()) do
            if towerFolder(child.Name) then out[#out + 1] = child.Name end
        end
        if #out == 0 then return nil, "No loaded tower to pick -- walk closer to one." end
        return out
    end

    -- Returns name, count -- or nil, nil, reason. `avoid` is skipped when there's anything
    -- else to pick, so re-rolling never hands you back the tower you already had.
    local rng = Random.new()
    local function randomTower(avoid)
        local names, why = loadedTowers()
        if not names then return nil, nil, why end

        local pool = names
        if avoid and #names > 1 then
            pool = {}
            for _, name in ipairs(names) do
                if name ~= avoid then pool[#pool + 1] = name end
            end
        end
        return pool[rng:NextInteger(1, #pool)], #names
    end
    ...
    -- The real folder name behind whatever is selected right now, or nil.
    local function selectedTower()
        local label = Options.PortalMatch and Options.PortalMatch.Value
        return label and labelToName[label] or nil
    end

    PortalBox:AddButton({
        Text    = "Pick Random Tower",
        Tooltip = "Select a random loaded tower. Click again to re-roll -- it won't hand you the same one back.",
        Callback = function()
            local name, count, why = randomTower(selectedTower())
            if not name then
            ...
            Library:Notify({
                Title       = "Tower Portal",
                Description = ("Picked %s (1 of %d loaded)."):format(name, count),
                Duration    = 3,
            })
        end,
    })

    -- Runs on the same 1s scan that refreshes the list. It only rolls when nothing valid
    -- is selected -- on first tick, or after the selected tower streams out as you move --
    -- so it isn't reshuffling the dropdown out from under you every second. Hit the button
    -- when you want a fresh one.
    local function autoPickTick()
        if not (Library.Toggles.AutoPickTower and Library.Toggles.AutoPickTower.Value) then return end
        if selectedTower() then return end

        local name = randomTower()
        if name then selectTower(name) end
    end

    PortalBox:AddToggle("AutoPickTower", {
        Text    = "Auto-Pick Random",
        Default = false,
        Tooltip = "Keep a random loaded tower selected -- re-rolls whenever the current pick streams out.",
    })
...
do
    ...
    local function setLabel(text)
        if AutoResetLabel then AutoResetLabel:SetText(text) end
    end

    -- Which tower are we standing in? Raycast down and walk the hit part up to
    -- workspace.Towers. Self-contained so this doesn't lean on the Floor Counter.
...
-- ===== Motion Blur (Visuals) =====
-- Roblox has no real motion blur, so this is the usual fake: a BlurEffect whose size
-- follows how fast the camera is actually turning and travelling, easing back to zero when
-- you stop. Driving it off camera movement is what makes it read as motion blur rather than
-- a permanently smeared screen -- standing still looks completely normal.
--
-- Its own function so its locals stay out of the main chunk's 200-local budget.
local function _initMotionBlur()
    local RunService = game:GetService("RunService")
    local Lighting   = game:GetService("Lighting")

    local blur, conn
    local current = 0     -- smoothed blur size, so it eases in and out instead of flickering
    local lastCF

    local function stop()
        if conn then conn:Disconnect() conn = nil end
        if blur then blur:Destroy() blur = nil end
        current, lastCF = 0, nil
    end

    -- Slider is 0-100 with 50 as "normal", so the default is a 1x multiplier.
    local function intensity()
        local slider = Library.Options.MotionBlurIntensity
        return ((slider and tonumber(slider.Value)) or 50) / 50
    end

    local function start()
        stop()
        -- Named, and only ever ours is destroyed: a place may ship its own BlurEffect and
        -- removing that would change how the game is supposed to look.
        blur = Instance.new("BlurEffect")
        blur.Name   = "PESMotionBlur"
        blur.Size   = 0
        blur.Parent = Lighting

        conn = RunService.RenderStepped:Connect(function(dt)
            local cam = workspace.CurrentCamera
            if not cam or not blur then return end
            if not blur.Parent then blur.Parent = Lighting end   -- something cleared Lighting

            local cf = cam.CFrame
            if not lastCF then lastCF = cf return end

            local turn = math.deg(math.acos(math.clamp(cf.LookVector:Dot(lastCF.LookVector), -1, 1)))
            local move = (cf.Position - lastCF.Position).Magnitude
            lastCF = cf

            -- A teleport is not movement. Autoplay tweens, respawns and tower entries jump
            -- the camera a long way in one frame, and blurring on those is just a flash.
            if move > 50 then
                current = 0
                blur.Size = 0
                return
            ...
            -- Rates per second, so the effect doesn't get stronger just because the frame
            -- rate dropped. Turning counts for more than travelling: it's what you notice.
            local step = math.max(dt, 1 / 240)
            local rate = (turn / step) * 0.05 + (move / step) * 0.03
            local target = math.clamp(rate * intensity(), 0, 40)

            current += (target - current) * math.clamp(dt * 12, 0, 1)
            blur.Size = current
        end)
    end

    local MotionGroup = Tabs.Visuals:AddRightGroupbox("Motion Blur")

    MotionGroup:AddToggle("MotionBlur", {
        Text    = "Motion Blur",
        Default = false,
        Tooltip = "Blurs the screen while the camera is turning or moving quickly, and clears when you stop.",
        Callback = function(state)
            if state then start() else stop() end
        end,
    })
    MotionGroup:AddSlider("MotionBlurIntensity", {
        Text     = "Intensity",
        Default  = 50,
        Min      = 0,
        Max      = 100,
        Rounding = 0,
        Tooltip  = "How strong the blur gets. 50 is normal, 0 switches it off in effect. Read live, so you can drag it while it's running.",
    })

    -- Clean the effect out of Lighting when the script is unloaded.
    task.spawn(function()
        repeat task.wait(1) until Library.Unloaded
        stop()
    end)
